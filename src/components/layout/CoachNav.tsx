...
const links = [
  { label: "Roster", href: "/coach" },
  { label: "Meetings", href: "/coach/meetings" },
  { label: "Availability", href: "/coach/availability" },
  { label: "Settings", href: "/coach/settings" },
];

export function CoachNav({ name }: { name: string }) {
  return (
    <header className="bg-brand-ink text-white">
      <nav className="container-page flex h-16 items-center justify-between gap-4">
        <Link href="/coach" className="shrink-0 text-lg font-bold tracking-tight">
          {siteConfig.shortName} Coach
        </Link>
